...
Use Java 21 for Android builds. The scripts default to `/usr/lib/jvm/java-21-openjdk`, but that path may not exist on this machine. A working local JDK 21 is installed at `/home/wooly/.local/share/jdks/jdk-21.0.11+10`; `build-apk.sh` auto-detects it, and `release-apk.sh` can use it with `JAVA_21_HOME=/home/wooly/.local/share/jdks/jdk-21.0.11+10 ./release-apk.sh <version>`.
